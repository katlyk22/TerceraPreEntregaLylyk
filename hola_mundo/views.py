...
from hola_mundo.models import Tarea, Persona, Animal
from hola_mundo.forms import PersonaForm, BuscarPersonasForm, AnimalForm
...
def crear_animal(request):
    ...
    if f.is_valid():
        Animal(nombre=f.data["nombre"], raza=f.data["raza"], subraza=f.data["subraza"], nacimiento=f.data["fecha_nacimiento"]).save()
        context['form'] = AnimalForm()

    context["animales"] = Animal.objects.all()
    context["total_animales"] = len(Animal.objects.all())
         
    return render(request, "hola_mundo/animales.html", context)
